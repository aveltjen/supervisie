...
error_reporting($ebits ^ E_NOTICE);
	...
	$tpl->loadTemplatefile("historiek.tpl.php");
	...
	if (isset($_REQUEST["selecteren"])) {
		...
		$selectie = $_REQUEST["selecteren"];
		$_SESSION["selecteren"] = $selectie;
		...
		foreach ($_REQUEST["selecteren"] as $msID){
			...
			$vorderingen = GetAllVorderingenDatumByMsID($msID,$WerfID);
			$vsnummer= 0;
			...
			while($vordering2 = $vorderingen2->fetchrow(MDB2_FETCHMODE_ASSOC)){
					...
					//TOTAAL van periode
					$totaal = 0;
					$vorderingen3 = GetAllVorderingenByPeriode($msID,$periode2,$WerfID);
