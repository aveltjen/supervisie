	...
	while($vs = $vslist->fetchrow(MDB2_FETCHMODE_ASSOC)){
		...
		if($vs["nummer"] != "" AND $vs["voorziene_hv"] != ""){
			...
			if($vs["vh_tb"] == "" AND $vs["eenheden"] == "")
			{
				...
				//if ($voorzien < $vs["totgevorderd"]) {
					//$tpl->setVariable("class","overschreden");
				//}else{
				$tpl->setVariable("class","vs");
				//}


				//$lastvordering = getLastVordering($WerfID);
	

				// if ($vs["id"] == $lastvordering["laatsteingave"]){
				// 	$tpl->setVariable("class","lastinsert");
				// }
