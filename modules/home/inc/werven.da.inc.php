...
<?php
        ...
        //Functie om werven op te halen aan de hand van  userID
        function GetWervenAllNotArchived(){
		...
		$db->query("UPDATE werven SET nummer = '$nummer', omschrijving = '$omschrijving', datum = '$startdatum' WHERE ID = '$id'");
    //print_r($db);
    }

		function updateLastVordering($meetstaatid,$werf){
			global $db;

		$db->query("UPDATE werven SET laatsteingave = '$meetstaatid' WHERE ID = '$werf'");
		}

		function getLastVordering($werf){
			global $db;

		$sql = $db->query("SELECT * FROM werven WHERE ID = '$werf'");
